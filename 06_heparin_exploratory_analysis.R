...
forest_plot <- function(data, title, color_variable, colors, output_file, height) {
  if (!color_variable %in% names(data)) {
    stop("Forest-plot grouping variable not found: ", color_variable)
  }
  comparison_levels <- unique(as.character(data$Comparison))
  plot_data <- data |>
    mutate(
      Comparison_plot = factor(as.character(Comparison), levels = comparison_levels),
      Plot_group = factor(as.character(.data[[color_variable]]), levels = names(colors))
    ) |>
    filter(
      is.finite(HR),
      is.finite(CI_low),
      is.finite(CI_high),
      !is.na(Comparison_plot),
      !is.na(Plot_group)
    ) |>
    arrange(Comparison_plot, Plot_group) |>
    mutate(
      Row_label = paste(as.character(Comparison_plot), as.character(Plot_group), sep = " | "),
      Label = sprintf("%.2f (%.2f-%.2f)", HR, CI_low, CI_high)
    )
  if (!nrow(plot_data)) stop("No finite estimates are available for forest plotting.")
  plot_data$Row <- factor(
    plot_data$Row_label,
    levels = rev(unique(plot_data$Row_label))
  )
  xmax <- max(plot_data$CI_high) * 1.8
  p <- ggplot(plot_data, aes(HR, Row, color = Plot_group)) +
